...
# Based on ERPNext Financial Report Template engine
...
from csf_ohada.csf_ohada.doctype.financial_report_row_enhanced.financial_report_row_enhanced import (
	FinancialReportRowEnhanced as FinancialReportRow,
)
...
class RowData:
	...
	column_values: dict[str, list[float]] = field(default_factory=dict)
	account_details: dict[str, AccountData] = field(default_factory=dict)
	column_account_details: dict[str, dict[str, AccountData]] = field(default_factory=dict)
	is_detail_row: bool = False
	parent_reference: str | None = None

	def get_all_numeric_values(self) -> list[float]:
		vals = []
		if self.column_values:
			for series in self.column_values.values():
				vals.extend(v for v in series if isinstance(v, int | float))
		else:
			vals.extend(v for v in self.values if isinstance(v, int | float))
		return vals
...
class FinancialReportEngine:
	...
	def _initialize_context(self, filters: dict[str, Any]) -> ReportContext:
		...
		template = frappe.get_doc("Financial Report Template Enhanced", template_name)

		if not template:
			frappe.throw(_("Financial Report Template Enhanced {0} not found").format(template_name))

		if template.disabled:
			frappe.throw(_("Financial Report Template Enhanced {0} is disabled").format(template_name))
		...
		measure_columns = get_measure_columns(template)
		has_custom_columns = bool(getattr(template, "columns", None))
...
class DataCollector:
	...
	def add_account_request(
		...
		self.account_requests.append(
			{
				"row": row,
				"accounts": self._parse_account_filter(
					self.company, row, calculation_formula=calculation_formula
				),
	...
	def collect_all_data(self) -> dict[str, Any]:
		...
		for request in self.account_requests:
			...
			column_code = request["column_code"]
			...
			column_summary.setdefault(ref_code, {})[column_code] = total_values
			column_account_details.setdefault(ref_code, {})[column_code] = request_account_details

			# Keep legacy keys pointing at default / first measure for compatibility
			if column_code == DEFAULT_MEASURE or ref_code not in summary:
		...
		return {
			...
			"column_account_details": column_account_details,
		}

	@staticmethod
	def _parse_account_filter(company, report_row, calculation_formula=None) -> list[dict]:
		"""
		...
		row_for_filter = report_row
		if calculation_formula is not None and calculation_formula != getattr(
			report_row, "calculation_formula", None
		):
			row_for_filter = frappe._dict(
				report_row.as_dict() if hasattr(report_row, "as_dict") else dict(report_row)
			)
			row_for_filter.calculation_formula = calculation_formula

		where_condition = filter_parser.build_condition(row_for_filter, account)
...
class RowProcessor:
	...
	def process_all_rows(self) -> list[RowData]:
		...
		column_summary = self.context.raw_data.get("column_summary", {})
		column_account_details = self.context.raw_data.get("column_account_details", {})
	...
	def _process_single_row(
		...
		if row.data_source == "Account Data":
			...
			return RowData(
				row=row,
				values=zeros,
				column_values={m.column_code: list(zeros) for m in self.measure_columns},
			)

	def _store_row_values(self, ref_code: str | None, column_values: dict[str, list]):
		if not ref_code:
			return
	...
	def _process_blank_row(self, row) -> RowData:
		blank = [""] * len(self.period_list)
		return RowData(
			row=row,
			values=list(blank),
			column_values={m.column_code: list(blank) for m in self.measure_columns},
		)

	def _process_column_break_row(self, row) -> RowData:
		return RowData(row=row, values=[], column_values={})
...
class FormulaCalculator:
	...
	def _resolve_series(self, values: Any, column_code: str) -> list:
		if isinstance(values, dict):
			if column_code in values:
				return values[column_code]
			if DEFAULT_MEASURE in values:
				return values[DEFAULT_MEASURE]
			if values:
...
class DataFormatter:
	...
	def _generate_columns(self) -> list[dict]:
		if self.context.has_custom_columns:
			base_columns = self._build_measure_columns()
		else:
		...
		columns = [
			{
				"fieldname": "account",
				"label": "Account",
				"fieldtype": "Link" if not company else "Data",
				"options": "Account" if not company else None,
				"width": 300,
			},
			{
				"fieldname": "currency",
...
class SegmentOrganizer:
	...
	def _should_show_row(self, row_data: RowData) -> bool:
		...
		if getattr(row, "hide_when_empty", False):
			significant_values = [val for val in row_data.get_all_numeric_values() if abs(flt(val)) > 0.01]
	...
	def _get_values(self, row_data: RowData) -> dict[str, Any]:
		...
		if self.context.has_custom_columns:
			for spec in iter_visible_value_columns(self.context.measure_columns, self.period_list):
				...
				values[spec["fieldname"]] = period_value
				if self.context.filters.get("accumulated_values") == 0 and isinstance(
		...
		if self.context.filters.get("accumulated_values") == 0 and row_data.row.fieldtype == "Percent":
			divisor = max(len(self.period_list), 1)
		...
		for segment in segments:
			for col in base_columns:
				...
				value_fieldnames = (
					...
					if self.context.has_custom_columns
					else [p["key"] for p in self.period_list]
				)
				if segment.label and col["fieldname"] in value_fieldnames:
...
class DetailRowBuilder:
	...
	def build(self) -> list[RowData]:
		...
		details_map = self.parent_row_data.account_details or {}
		column_details = self.parent_row_data.column_account_details or {}
		if not details_map and column_details:
			details_map = column_details.get(DEFAULT_MEASURE) or next(iter(column_details.values()), {})
...
class ChartDataGenerator:
	...
	def generate(self) -> dict[str, Any]:
		...
		# Prefer first visible custom measure; otherwise legacy values
		chart_measure = None
		if self.context.has_custom_columns:
		...
		for row_data in chart_rows:
			...
			series = row_data.values
			if chart_measure and row_data.column_values:
				series = row_data.column_values.get(chart_measure) or series

			values = []
			for i, _period in enumerate(self.period_list):
				if i < len(series):
					value = series[i]
					values.append(flt(value, 2) if isinstance(value, int | float) else 0.0)
				else:
					values.append(0.0)
	...
	def transform(self) -> None:
		...
		for row_data in self.formatted_rows:
			...
			for measure in measures:
				for i in range(len(self.period_list)):
					if has_custom:
						fieldname = make_fieldname(measure.column_code, self.period_list[i]["key"])
						prev_fieldname = (
							make_fieldname(measure.column_code, self.period_list[i - 1]["key"])
							if i != 0
							else None
						)
					else:
						fieldname = self.period_list[i]["key"]
						prev_fieldname = self.period_list[i - 1]["key"] if i != 0 else None

					current_value = row_data.get(fieldname)
					previous_value = row_data.get(prev_fieldname) if prev_fieldname else 0

					if i == 0:
						transformed_values[fieldname] = current_value
					else:
						transformed_values[fieldname] = self._calculate_growth(previous_value, current_value)
