...
def get_account_type_based_gl_data(company, filters=None):
	...
	if filters.include_default_book_entries:
		company_fb = frappe.db.escape(frappe.get_cached_value("Company", company, "default_finance_book"))
		finance_book = frappe.db.escape(filters.finance_book)
		cond = f" AND (finance_book in ({finance_book}, {company_fb}, '') OR finance_book IS NULL)"
	else:
		finance_book = frappe.db.escape(cstr(filters.finance_book))
		cond = f" AND (finance_book in ({finance_book}, '') OR finance_book IS NULL)"
